...
**业务规则配置见 `references/constraints.json`（单一事实来源）**，包括：
- 优先问题数量上限
- READY 状态条件
- 决策账本约束
- 资料状态分层规则
...
3. **反自洽原则**（见 `references/constraints.json` 的 antiConsistencyRules）：category 为 `user_material`（用户显式上传）或 `historical_prd`（历史 PRD）的来源，adoption 不得标记为 `default_adopt`（草稿不能当已上线事实）。
...
5. 优先问题不超过配置上限（见 `references/constraints.json`），避免用大量低价值问题阻塞写作。
6. 不输出 PRD 标题、章节正文、功能列表或验收条款（见 `schema.json` 输出契约）。
