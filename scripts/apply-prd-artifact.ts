...
export function applyPrdArtifact(taskId: string, outputPath: string, root = PROJECT_ROOT) {
  ...
  const writeResult = writePrdArtifactFile(output, root, nowISO());
  ...
  appendEvent({
    ...
    details: { phase: artifact.phase, version: artifact.version, status }
  });
  return { status, phase: artifact.phase, version: artifact.version, artifact_ref: pathToRepoRef(targetPath, root) };
}
