...
function main() {
  ...
  const targetPath = path.join(PROJECT_ROOT, "context-workspace/workspace/decisions/decision-ledger.json");
  writeJsonAtomic(targetPath, ledger);
  state.decision_ledger_version = ledger.version;
  state.latest_output_ref = pathToRepoRef(targetPath);
  ...
  appendEvent({
    ...
    prompt_version: "0.2.0", artifact_ref: pathToRepoRef(targetPath),
    details: { version: ledger.version, confirmed_decision_ids: confirmedIds }
  });
  console.log(JSON.stringify({ status: "RECORDED", ledger_ref: pathToRepoRef(targetPath), version: ledger.version }, null, 2));
}
