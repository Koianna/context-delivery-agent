...
Context 路径按项目隔离：所有项目统一使用 `context-workspace/projects/<project_id>/context/`。状态机只认当前任务 `project_id` 对应的 Context 索引和版本；评测使用 `evaluation/fixtures/` 中的隔离通用夹具，不作为运行时业务数据源。
