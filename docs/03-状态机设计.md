...
状态机的完成语义是强约束：`WAITING_*`、`TASK_PAUSED`、`EXECUTION_BLOCKED` 和 `TASK_CANCELLED` 都不是业务完成。外部 Agent 不能把这些状态改写成“已整理”或“已交付”，也不能在这些状态下自行生成替代文件。只有进入 `CONTEXT_TASK_COMPLETED` 或 `DELIVERED`，且 Runtime 已登记业务 artifact，任务才可向用户报告完成。

### 1.1 适配层不是业务状态

MCP Adapter、JSONL Gateway 和 CLI Adapter 都是同一 Runtime 的接入方式，不新增状态、不拥有状态转移权。无论请求来自哪个宿主，Runtime 都必须先读取 `runtime/task-state.json`，再按当前状态决定是否调用 Skill 或处理确认。内联材料与本地路径材料进入同一条 `CONTEXT_ANALYZING` 状态路径。
...
| 等待重复材料重新整理确认 | `WAITING_MATERIAL_REPROCESS_CONFIRM` | 等待 | 检测到相同材料已整理过，等待是否重新整理并覆盖整理稿 |
...
| 等待 PRD 产物恢复决定 | `WAITING_PRD_RECOVERY_CONFIRM` | 等待 | PRD 缺失或完整性冲突且不能自动恢复，等待重新对齐或取消 |
...
stateDiagram-v2
    ...
    PRD细节补充中 --> PRD独立审核中
    PRD细节补充中 --> 等待PRD产物恢复决定: PRD无法自动恢复
    PRD独立审核中 --> 等待审核处理决定
    PRD独立审核中 --> 等待PRD产物恢复决定: PRD无法自动恢复
    等待PRD产物恢复决定 --> PRD写前对齐中: 重新开始写前对齐
    等待PRD产物恢复决定 --> 任务已暂停: 暂停
    等待PRD产物恢复决定 --> 任务已取消: 取消
...
### 4.4A 等待重复材料重新整理确认 `WAITING_MATERIAL_REPROCESS_CONFIRM`

进入条件：当前任务提交的材料内容哈希与同一项目其他任务已登记材料相同。Runtime 必须返回已有整理稿引用并询问用户是否重新整理并覆盖，不能自动跳过或覆盖。

允许动作：`APPROVE_REPROCESS`、`KEEP_EXISTING`。批准后重新执行 `CONTEXT_ANALYZING`，并复用已有整理稿路径；保留后不重新分析、不覆盖已有整理稿。该确认只影响可阅读整理稿和本次分析，不会绕过 CP-C01 修改稳定 Context。
...
进入条件：PRD 主体已确认、重规划只影响细节层，或 CP-P03 已选择 `FIX_AND_REVIEW` 且用户提交了具体审核修订决定。
...
- 保存新的 PRD 版本；
- 标记未确认数据。
- 在审核修订场景调用 `prd-write/REVISION`，基于当前 PRD、当前审核报告和用户决定生成补丁版本；不得复用旧 DETAILS 输出。
...
进入条件：PRD 细节完成，或用户在 CP-P03 选择先修复后等待提交具体修订决定。
...
- 输出 P0/P1/P2 问题；
- 保存审核版本。
- 在上一条 CP-P03 的 resolution 为 `FIX_AND_REVIEW` 时，接收具体修订决定并进入 `PRD_DRAFTING_DETAILS`；普通“继续”或含糊回复只提示补充决定，不创建意图澄清确认。

合法下一状态：

- `WAITING_REVIEW_DECISION`；
- `PRD_DRAFTING_DETAILS`（仅审核修订决定已明确）；
- `EXECUTION_BLOCKED`。
...
- 用户选择 `FIX_AND_REVIEW`：关闭当前 CP-P03 并进入 `PRD_REVIEWING`；用户给出具体修订决定后生成新版本、重新审核并创建新的 CP-P03。

### 4.15A 等待 PRD 产物恢复决定 `WAITING_PRD_RECOVERY_CONFIRM`

进入条件：审核修订前的完整性检查发现 PRD 正文缺失、版本或 hash 冲突、恢复快照损坏，且 Runtime 不能安全地自动恢复原字节。

规则：

- 有效恢复快照存在且正文仅为缺失时，Runtime 直接恢复并记录 `ARTIFACT_RECOVERED`，不进入本状态；
- 当前正文与快照或审核 hash 不一致时不得静默覆盖；
- 旧任务没有恢复快照时不得根据审核报告或模型输出补造原 PRD；
- 用户选择 `RESTART_PRD_ALIGNMENT` 后在同一 `task_id` 返回 `PRD_THINKING`，重新创建 CP-P01、CP-P02 和 CP-P03；旧确认只保留为历史；
- 用户也可以暂停或取消任务。
...
当最近有效状态为 `CONTEXT_ANALYZING` 且任务模式为 `CONTEXT` 时，用户回复“重试”必须重新调用材料接入和 Context 分析步骤；不得只把状态写回执行态后等待下一条输入，否则会形成 `EXECUTION_BLOCKED ↔ CONTEXT_ANALYZING` 循环。

阻塞态被暂停时，Runtime 必须把阻塞前的业务执行态保存在 `return_state`。旧任务缺少该字段时，可从 `completed_steps` 反推最近执行态；恢复顺序为 `TASK_PAUSED → EXECUTION_BLOCKED → 最近执行态`，不得把 `TASK_PAUSED` 误当成业务重试节点。
...
POC 边界：取消只关闭当前任务并写入取消事件，不删除文件、不回滚 Git；已执行的 Context 写入如需撤销，必须另行发起明确的 Context 撤销请求，经 CP-C01 批准后以 `ARCHIVE` 标记归档并从索引隐藏。若用户要继续原任务，应创建新任务；已取消任务本身不得直接恢复。
...
9. 外部 Agent 只能提交用户自然语言和原始材料，不能提交目标状态、批准动作、确认记录 ID 的替代值或内部 Skill 名称。
10. Adapter 进程中断、宿主替换或 MCP 重连不改变状态机；恢复时以 `runtime/` 为准，不能根据外部 Agent 的上下文记忆补造状态。
11. 只有 Runtime 返回的状态、Skill 调用记录和产物引用才能作为任务已执行的证据；外部 Agent 未调用工具时不得宣称任务完成。
12. 创建确认与进入对应等待态必须由同一 Runtime 操作完成。状态迁移失败时，刚创建的确认保留历史但标记为 `CANCELLED`、`resolved_by = SYSTEM`，不得留下孤立 `PENDING`。
13. 每次恢复或重试都要取消未被 `task_state.pending_confirmation` 引用的悬空 `PENDING`；当前等待态引用的确认不得被清理。`TASK_PAUSED` 暂时不执行孤立清理，恢复到原等待态后必须重新关联原确认。
...
- `runtime/pending-confirmations.json`：确认历史及当前未完成确认记录；等待状态必须能找到与 `pending_confirmation` 一致的唯一 `PENDING`；
...
Context 路径按项目隔离：所有项目统一使用 `context-workspace/context/<project_id>/`。状态机只认当前任务 `project_id` 对应的 Context 索引和版本；评测使用 `evaluation/fixtures/` 中的隔离通用夹具，不作为运行时业务数据源。
...
- 有意义的里程碑创建 Git checkpoint commit；
