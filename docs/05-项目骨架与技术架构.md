...
> 阶段：个人 POC  
> 更新日期：2026-08-03  
> 上游文档：《产品定义与 POC 范围》v1.1、《用户任务流程》v0.2、《Agent 状态机》v0.2、《输入输出契约》v0.2  
...
> 外部 Agent 是可替换的自然语言交互宿主；External Agent Gateway 是协议适配层；项目 Runtime（`AgentOrchestrator`、状态机、六个 Skill 和 TypeScript Harness）是唯一的业务编排与执行中心；文件型 Context 仓库保存业务材料和交付产物；本地 Runtime 保存即时任务状态；Git/GitHub 负责版本追溯、评测绑定和脱敏作品展示。
...
flowchart LR
    ...
    EA --> GW["External Agent Gateway"]
    GW --> A["AgentOrchestrator / Runtime"]
...
### 2.1 外部 Agent 宿主与 Gateway

外部 Agent 宿主负责自然语言理解、追问、确认收集和结果展示，可以被替换。Gateway 只负责 JSONL 请求校验、调用 `AgentOrchestrator.handleMessage()` 和响应包装，不直接读写业务文件、不操作状态机、不关闭确认记录。
...
POC 不把某个模型厂商或桌面产品写成不可替换的业务依赖。只要 Runtime 支持规则文件、文件读写、命令执行和 Skill 调用，即可替换实现。
...
POC 的通用项目使用 `context-workspace/projects/<project_id>/context/` 保存独立 Context；帮助中心搜索演示案例继续使用 `context-workspace/context/`，以保持案例、评测和历史产物可复现。通用材料只有在 CP-C01 批准后才创建或更新稳定 Context 文件。
...
- 产品文档与架构决策；
- 脱敏后的 Context、通用项目材料和帮助中心搜索演示案例；
...
`runtime/*.json` 和 `runtime/*.jsonl` 是运行时生成文件，应加入 `.gitignore`。仓库中通过 Schema、示例和初始化脚本说明格式，不提交真实会话状态。

---

## 7. 模块调用与状态映射

所有业务调用都由 Runtime 编排器发起。外部 Agent 只能调用 Gateway，不能直接调用下表的 Skill 或 Harness。
...
### 阶段 7：External Agent Gateway

- 定义外部 Agent 请求/响应契约；
- 实现 JSONL Gateway；
- 验证 Gateway 与参考 CLI 都复用同一个 Runtime 编排结果；
- 验证外部宿主替换时不改变状态、确认门禁和产物写入；
- 未来接入 MCP、HTTP 或 IDE 时仅新增适配器，不复制业务编排。
