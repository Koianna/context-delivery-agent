...
POC 的通用项目使用 `context-workspace/context/<project_id>/` 保存独立 Context。项目不保留固定业务案例；所有真实材料只有在 CP-C01 批准后才创建或更新稳定 Context 文件，Context 项目目录和分类子目录也在实际写入时按需创建，回归夹具只在临时目录使用。
