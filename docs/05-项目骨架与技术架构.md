...
> 本地主 Agent Runtime 负责理解目标和动态编排；`AGENTS.md` 定义全局规则、Context 加载和 Skill 路由；六个原生 Skill 负责专业任务；TypeScript Harness 脚本负责状态、确认、写入、版本和日志的确定性约束；文件型 Context 仓库保存业务材料和交付产物；本地 Runtime 保存即时任务状态；Git/GitHub 负责版本追溯、评测绑定和脱敏作品展示。

本项目不采用三个完全自治的子 Agent，也不把六个 Skill 简化为固定流水线。主 Agent 可以根据用户目标、信息成熟度和当前状态选择下一步，但不能绕过 Harness 直接修改状态或稳定 Context。
...
### 2.1 本地 Agent Runtime

POC 通过具备本地文件和命令调用能力的 Agent Runtime 运行，不开发独立 WebApp。Runtime 承担：

- 接收用户消息和本地材料路径；
- 加载仓库根目录的 `AGENTS.md`；
- 允许主 Agent 读取 Context、调用 Skill 和执行受控脚本；
- 展示当前状态、产物摘要和人工确认项；
- 支持同一仓库内的暂停、退出和跨会话恢复。
...
Runtime 文件由脚本生成和维护。主 Agent只能通过 Harness 读取或修改，不直接自由编辑。
