...
MCP、JSONL 和 CLI 仅是不同接入层，均调用同一个 `AgentOrchestrator`；接入层不产生额外业务状态。内联材料先进入 `context-workspace/drafts/<project_id>/source-materials/`，之后与本地路径材料共用 `CONTEXT_ANALYZING` 路径。
...
Context 分析阶段除 JSON 分析报告外，Runtime 必须生成一份可阅读的结构化材料整理稿。会议材料写入 `context-workspace/workspace/agent-runs/<task_id>/materials/meeting-note.md`，其他材料写入同目录的 `structured-materials.md`。会议材料同时兼容带发言人标记和无发言人连续段落两种输入；该文件属于 Runtime artifact，外部 Agent 只能展示其引用，不能在根目录另建 `meeting-notes/`。
