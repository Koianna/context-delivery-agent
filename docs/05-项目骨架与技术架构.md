...
`README.md` 包含主 Agent 的全局执行规则，至少定义：
...
全局规则负责路由和约束，不承载某一个 Skill 的详细方法。专业步骤放在各 Skill 的 `SKILL.md` 和 Prompt 中。
...
- `README.md` 核心概念、Prompt、Skill、Schema 和脚本版本；
- 产品文档与架构决策；
...
2. 按 README.md 规则加载最小必要 Context
3. 在当前状态下解释用户输入
...
`context-workspace/workspace/prd-recovery/<artifact_id>/` 仅在 PRD 首次成功写入后按需创建。目录使用 PRD 完整文件的 SHA-256 作为快照文件名，`manifest.json` 记录任务、版本、正文 hash、文件 hash、目标引用和快照引用。Runtime 只有在目标 PRD 缺失且快照、版本及最近审核 hash 全部匹配时才自动恢复；内容冲突或旧任务没有快照时进入 `WAITING_PRD_RECOVERY_CONFIRM`，不会提前创建空目录，也不会生成 v1/v2/final 业务副本。该目录属于可重建的 Runtime 恢复记录，不纳入 Git。
...
MCP、JSONL 和 CLI 仅是不同接入层，均调用同一个 `AgentOrchestrator`；接入层不产生额外业务状态。内联材料和本地路径材料统一落盘到 `context-workspace/drafts/<project_id>/source-materials/<task_id>/materials.md`，每次任务输入只生成这一个 Markdown 原文包，包内以 `source_id` 边界保留多份原文。任务级元信息（`task_id`、`task_goal`、`registered_at`、`updated_at`、`structured_material_ref`）写在 bundle 顶部 YAML frontmatter；每条材料的属性写在 `<!-- context-material: key=value | ... -->` 头注释里。**Bundle 是唯一真相源**。项目级 `material-manifest.json` 作为派生缓存保存到 `.cache/manifests/<project_id>/`（gitignore），持有 `ingestions`（每次任务的接入元数据）和 `records`（逐条逻辑来源、哈希与共享 draft 引用），缓存丢失时 Runtime 会从 drafts bundle 反扫零损重建，不复制原文。`sha256` 只对 bundle 正文（去 frontmatter 后）计算，产品经理修改 frontmatter 不会污染去重。多个记录可以指向同一个 `materials.md`，但分析、证据校验和重复检测必须按各自 `source_id` 读取并计算。旧版本的一文件一材料目录和任务目录中的 `ingest-manifest.json` 仅用于兼容读取，不再由新流程生成。
...
| 等待 PRD 产物恢复决定 | 无 | `manage-confirmation`、PRD 完整性校验、`transition-state`；重新对齐或取消 |
...
Context 分析阶段除 JSON 分析报告外，Runtime 必须生成一份可阅读的结构化材料整理稿。分析报告保存在被 Git 忽略的 `.cache/agent-runs/<task_id>/`，通过 `runs://<task_id>/...` ref 引用；会议整理稿发布到 `context-workspace/workspace/projects/<project_id>/materials/meeting-notes/<task_id>.md`，其他整理稿发布到同项目的 `materials/structured-materials/<task_id>.md`。整理稿属于中可信度 Runtime artifact，可纳入版本管理，但不得自动提升为稳定 Context。外部 Agent 只能展示 Runtime 返回的引用，不能在根目录另建 `meeting-notes/`。

稳定 Context 的章节级降级沿用 `CONTEXT_ANALYZING → WAITING_CONTEXT_CONFIRM → CONTEXT_MAINTAINING`。Runtime 按 Markdown 标题边界提取用户指定章节，把剩余正文写入 `.cache/agent-runs/<task_id>/candidates/`（`runs://<task_id>/candidates/...`），把移出内容写入 `workspace/projects/<project_id>/context-pending/<task_id>/`，并生成 `UPDATE_CONTEXT` proposal。CP-C01 批准后由 `create-version` 校验基线并递增原文件版本，`update-index` 保持文件 active。派生文件 `INDEX.md` 不得成为撤销或局部更新 proposal 的目标。

共 24 个状态均在此表登记。等待状态统一通过 `manage-confirmation` 创建或处理确认记录；执行状态先调用对应 Skill，再由 `transition-state` 校验下一状态；结束、暂停、阻塞和取消状态不得继续调用业务 Skill。两个业务终态的版本和索引动作必须在入态前完成，不在终态中重复执行。
...
- 核心概念、六个 Skill、Harness、Context、Runtime 和评测目录边界清楚；
