...
Runtime 将真实模型接入拆成两层：`OpenAIProvider` 负责 Context、PRD、Change 的业务输入整理和结果映射，`StructuredModelClient` 只负责“指令 + JSON Schema + 材料 → JSON”的模型调用。当前内置 `OpenAIResponsesClient`、OpenAI 兼容 Chat Completions 客户端和 `AnthropicMessagesClient`，分别覆盖 OpenAI、DeepSeek/Kimi/其他兼容服务和 Claude。通过 `MODEL_PROVIDER` 选择；未启用时 Runtime 使用本地 `WorkspaceProvider`。模型接入只替换生成层，不改变以下控制权：
...
真实模型客户端统一要求结构化 JSON。OpenAI Responses 使用严格 JSON Schema；OpenAI 兼容服务使用 `response_format=json_object`；Claude 使用 Messages API 并在提示中传入结构要求，再由本地校验器兜底。材料整理阶段发送当前任务原始材料；PRD 阶段发送当前项目必要的 Context 和分析产物；变更阶段发送变更请求和受影响业务产物。使用者必须在启用真实模型前判断这些材料是否允许发送给外部模型服务。
