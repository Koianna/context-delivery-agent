...
Context 分析阶段除 JSON 分析报告外，Runtime 必须生成一份可阅读的结构化材料整理稿。会议材料写入 `context-workspace/workspace/agent-runs/<task_id>/materials/meeting-note.md`，其他材料写入同目录的 `structured-materials.md`。该文件属于 Runtime artifact，外部 Agent 只能展示其引用，不能在根目录另建 `meeting-notes/`。
